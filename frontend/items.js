...
function generateItemmHTML(item) {
  ...
  <div class="product-qty">${item.productqty}</div>
...
function displaySimilarProducts() {
  ...
  let similarProducts = items.filter(item =>
  ...
  let priceSimilarProducts = items.filter(item =>
    Math.abs(item.price.currentPrise - currentProduct.price.currentPrise) <= priceThreshold && // Similar price
    item.id !== currentProduct.id && // Different product
